...
@Component({
  selector: 'app-perfectionement',
  templateUrl: './perfectionement.component.html',
  styleUrls: ['./perfectionement.component.css']
})
export class PerfectionementComponent implements OnInit {
  ...
  private loadClassesAndEtudiants(): void {
    this.classeService.getClassesByAnnee('2eme_annee').subscribe({
      next: (classes) => {
 ...
 private loadInitialData(): void {
  this.loading = true;
  this.message = null;
  this.affectationService.getPerfectionnementAffectations().subscribe({
    next: (affectations) => {
  ...
  generateAffectations(): void {
    this.loading = true;
    this.message = null;
    this.affectationService.generateAffectations('perfectionnement').subscribe({
      next: (affectations) => {
        this.affectations = affectations;
 ...
 removeEtudiant(affectationId: string, etudiantIndex: number): void {
  ...
  exportToExcel(): void {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Affectations Perfectionnement');
    
    ws['!cols'] = [
      { width: 40 }, { width: 15 }, { width: 30 }, { width: 15 }, { width: 20 }
    ];
    
    XLSX.writeFile(wb, `Affectations_Perfectionnement_${new Date().toISOString().slice(0,10)}.xlsx`);
  }
}